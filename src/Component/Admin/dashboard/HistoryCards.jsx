...
const HistoryCards = ({ handleClose }) => {
  ...
  const handleRequestAction = async (action, id, reason = "") => {
    const token = sessionStorage.getItem("token");
    const userName = sessionStorage.getItem("Name");
    const isApprove = action === "approve";

    // Construct the correct URL based on action
    const url = isApprove
      ? `${
          Config.API_BASE_URL
    ...
    try {
      ...
      alert(
        `Error occurred while ${action} the request. Please try again later.`
      );
    }
  };

  // Open dialog to get reason before denying
  const handleOpenDenyDialog = (id) => {
  ...
  const handleSubmitDeny = () => {
    if (!denyReason.trim()) {
      alert("Please provide a reason before denying.");
      return;
    }
    handleRequestAction("deny", selectedRequestId, denyReason);
    handleCloseDialog();
  };
    ...
    <Box sx={{ p: { xs: 1, sm: 2 } }}>
            ...
            >
              ...
              <CardContent sx={{ flexGrow: 1 }}>
                <Stack spacing={1.5}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {new Date(request.localDateTime).toLocaleDateString()}
                    </Typography>
                  </Stack>
                  <Divider sx={{ my: 1 }} />
                  <Box
              ...
              <CardActions sx={{ justifyContent: "flex-end", p: 2 }}>
                <Button
                  size="small"
                  color="error"
                  onClick={() => handleOpenDenyDialog(request.id)}
                >
                  Deny
                </Button>
                <Button
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmitDeny} color="error" variant="contained">
            Submit
          </Button>
        </DialogActions>
      </Dialog>
